...
class MultiQueryAttention(Attention):
    ...
    def __init__(self, word_size: int = 512, embed_dim: int = 64, n_query:int=8) -> None:
        ...
        self.proj = torch.nn.Linear(in_features=embed_dim * n_query,
                              out_features=embed_dim, bias=False)
        delattr(self, 'query')
        self.querys = torch.nn.ModuleList([
            torch.nn.Linear(in_features=word_size, out_features=embed_dim, bias=True)
            for _ in range(n_query)
        ])
        self.key = torch.nn.Linear(in_features=word_size, out_features=embed_dim, bias=True)
        self.value = torch.nn.Linear(in_features=word_size, out_features=embed_dim, bias=True)
    ...
    def self_attention(self, Q: Tensor, K: Tensor, V: Tensor,
        ...
        * [Memory-efficient attention](https://facebookresearch.github.io/xformers/components/ops.html)
        ...
        """
        loss_of_one_batch(collate_with_cat(pairs[i:i + batch_size]), model, None, device)
        return Z
